...
# ===============================
try:
    # This allowlists the specific numpy scalar that is causing your error
    from torch.serialization import add_safe_globals
    # Handle different numpy versions naming conventions
    if hasattr(np, '_core'):
...
class ModelManager:
    def __init__(self):
        ...
        # CORRECTED PATH: Points to the 'data/models' subfolder as shown in your image
    ...
    def load_models(self):
        try:
            ...
            if not seg_path.exists():
                logger.error(f"❌ Model not found at {seg_path}")
                return

            self.segmentation_model = UNet3D(
                in_channels=1,
                out_channels=3,
                init_features=24,   # ✅ MATCH TRAINING
                use_attention=False
            ).to(self.device)
