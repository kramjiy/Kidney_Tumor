...
class KidneyTumorAnalyzer:
    ...
    def __init__(self):
        ...
        if model_path.exists():
            ...
            self.segmentation_model = UNet3D(
                in_channels=1, 
                out_channels=3, 
                init_features=32,  # Match your training config
                use_attention=False
            ).to(self.device)
    ...
    def predict_growth(self, detection, radiomics_features):
        ...
        # Aggressive tumors: 0.5-1.5 cm/year
        # Indolent tumors: 0.1-0.3 cm/year
...
def load_pretrained_models():
    ...
    try:
        analyzer.segmentation_model.load_state_dict(
            torch.load('best_segmentation_model.pth', map_location=analyzer.device, weights_only=False)
        )
    ...
    try:
        analyzer.classification_model.load_state_dict(
            torch.load('best_classifier_model.pth', map_location=analyzer.device)
        )
    ...
    try:
        analyzer.growth_model.load_state_dict(
            torch.load('best_growth_model.pth', map_location=analyzer.device)
        )
